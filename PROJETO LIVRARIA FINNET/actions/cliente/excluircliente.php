...
if(isset($_GET["id"]) && !empty($_GET["id"])){
    ...
    if($resultado)
{
    header("Location ./cadastrarcliente.php?mensagem=Excluido com sucesso");
    exit();
}else{
    header("Location ./cadastrarcliente.php?mensagem=Ocorreu algum erro");
    exit();
}
}else{
    header("Location: ./cadastrarcliente.php?mensagem=Selecione um usuario para apagar");
    exit();
}
?>