...
if(isset($_GET["id"]) && !empty($_GET["id"])){
    ...
    $query = "Delete from locacao where id=".$_GET["id"];
    $resultado = mysqli_query($conexao, $query);
    if($resultado)
{
    header("Location ./locacaolivro.php?mensagem=Excluido com sucesso");
    exit();
}else{
    header("Location ./locacaolivro.php?mensagem=Ocorreu algum erro");
    exit();
}
}else{
    header("Location: ./locacaolivro.php?mensagem=Selecione um livro para apagar");
    exit();
}
?>